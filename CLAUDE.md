...
- **공통 인터페이스를 바꾸는 순서는 `docs/api.md` 수정 → 관련 담당자 확인 → 코드 수정입니다. 코드를 먼저 고치지 마세요.**
...
**`frontend/`와 `backend/`는 각각 여러 명이 함께 쓰는 폴더입니다.** 한 사람의 폴더가 아닙니다.
`backend/`는 역할 3·4·7, `frontend/`는 역할 5·6이 동시에 작업합니다 ([PLANNING §9](docs/PLANNING.md#9-역할-7명)).

- **읽기는 어디든 허용됩니다.** 다른 영역의 코드를 읽어 규약을 파악하는 것은 권장됩니다.
- **쓰기는 지금 맡은 영역을 우선합니다.** 같은 폴더 안이라도 **다른 담당자의 파일**을 고쳐야 하면
  먼저 사용자에게 알리고, 최소 범위로만 손댑니다.
